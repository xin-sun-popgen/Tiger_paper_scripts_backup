...
for SHORT_NAME in $(cat name_se)
do
    samtools view -h ${SHORT_NAME}_wg_f_sort_rmdup.bam | awk '{if(match($1, "@")){print $0}else{if(match($6,"S")){}else{print $0}}}' OFS='\t' > ${SHORT_NAME}_wg_f_sort_rmdup_se.sam
